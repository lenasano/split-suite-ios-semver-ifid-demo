...
@MainActor
class CoffeeDataAsync: ObservableObject {
    ...
    static let shared = CoffeeDataAsync()
    lazy var healthKitController = HealthKitControllerAsync(withModel: self)
    
    // An actor used to save and load the model data away from the main thread.
    private let store = CoffeeDataStoreAsync()
    ...
    public var currentMGCaffeineString: String {
        if #available(iOS 15, *) {
            ...
            return currentMGCaffeine.formatted(floatFormatter)
            ...
            return formatter.string(for: currentMGCaffeine)!
        }
    }
    ...
    public func mgCaffeineString(atDate date: Date) -> String {
        if #available(iOS 15, *) {
            ...
            return mgCaffeine(atDate: date).formatted(floatFormatter)
            ...
            return formatter.string(for: mgCaffeine)!
        }
    }
    ...
    public var totalCupsTodayString: String {
        if #available(iOS 15, *) {
            ...
            return totalCupsToday.formatted(floatFormatter)
            ...
            return formatter.string(for: totalCupsToday)!
        }
    }
